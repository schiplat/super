...
- **Dependency vulnerabilities** — `cargo audit` runs against the RustSec advisory database; release branches must be clean of known-vulnerable dependencies before tagging. As of `1.3.0` the scan reports **no vulnerabilities** in release binaries. Remaining advisories are low-severity warnings in dev-only dependencies (`lru` via CLI TUI) and do not ship in `superd` release binaries.
