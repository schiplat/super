...
- **Flapping no longer fires on dependency waits**: start attempts that stop at the `depends_on` gate do not count toward flapping; only real spawn attempts do.
- **Held-stopped dependencies stay stopped**: a program stopped with `super stop` is not revived by a dependent's auto-start path.
- **`start_period_secs` is a grace window**: probes run immediately after start; failures inside the window do not count toward `max_failures`, and the first success ends the grace early.

### Changed

- Positioning and docs: edge / cloud-edge focus (no IoT claim); extension docs corrected for real hook call sites (`before_stop` reserved, `after_stop` on exit + remove, `after_start` fail-secure).
- Roadmap: AI-agent infrastructure under P1; `delay_secs` and `before_stop` wire-or-remove under P2; P0 import work sequenced behind a `StackFormat` parser layer.

### Notes

- Workspace **1.5.6**; pair with commercial plugin packages `super-plugins-1.5.6-…`.
