...
| Adds | Core process management, OTA, health checks, cron, event hooks, **embedded Dashboard shell** | API auth/RBAC/audit (`security`), cgroups limits (`isolation`, Linux), notifications (`notify`), Dashboard Pro extensions (`ui` plugin) |
